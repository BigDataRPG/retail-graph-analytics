...
1.  Start a Neo4j database instance.
2.  Set the following environment variables (or configure them in `app/tools.py` and `app/neo4j_ingest.py`):
    ```bash
...
3.  Ingest the data:
    ```bash
    # From retail-graph-analytics directory
    python app/neo4j_ingest.py
    ```
